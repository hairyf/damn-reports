...
pub async fn post_collect(State(state): State<AppState>) -> (StatusCode, Json<serde_json::Value>) {
    ...
    tokio::spawn(async move {
        match task::collect_records_of_source::trigger(db_clone).await {
            Ok(count) => {
                log::info!(
                    "Background collect task completed successfully, collected {} records",
                    count
                );
            }
            Err(e) => {
