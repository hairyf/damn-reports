...
```json
{
  ...
  "jobs": [
    ...
    },
    {
      "id": "my_report_end",
      "name": "日报后通知",
      "schedule": { "kind": "reportend", "trigger": "every" },
      "payload": { "kind": "command", "command": "node tools/notify.js" },
      "files": ["tools/notify.js"],
      "state": {},
      "createdAtMs": 1739000000000,
      "updatedAtMs": 1739000000000
    }
...
- `schedule`（object）
  ...
  **日报生成后**（reportend，事件驱动）：
  ```json
  { "kind": "reportend", "trigger": "every" }
  ```
  - `trigger`：`every` = 每次日报生成后执行；`scheduled` = 仅定时任务触发时报生成后执行
  - 无 `nextRunAtMs`，由日报生成事件触发。执行命令由 `payload` 的 `command` 指定。
...
- `payload`（object）
  ...
  **AI 对话**（发送消息到主会话）：
  ```json
  { "kind": "mainagent", "message": "储存今天的记忆" }
  ```
  ...
  **执行命令**：
  ...
  ```
  - 当 schedule 为 `reportend` 时，`payload` 也使用 `command` 类型，报告文件路径会作为参数追加：`{command} "<reportPath>"`。
...
示例：`schedule: {"kind": "reportend", "trigger": "every"}`, `payload: {"kind": "command", "command": "node tools/notify.js"}` → 日报生成后执行 `node tools/notify.js "<报告文件路径>"`，脚本可通过首个参数读取报告内容。
