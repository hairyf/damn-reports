/**
 * Apply patch tool - OpenAI-style *** Begin Patch / *** End Patch format
 * Adapted from openclaw sources/openclaw/src/agents/apply-patch.ts
 */
...
function toDisplayPath(resolved: string): string {
...
export async function applyPatch(input: string): Promise<ApplyPatchResult> {
  ...
  for (const hunk of parsed.hunks) {
    const resolved = resolvePath(hunk.path)
    const display = toDisplayPath(resolved || hunk.path)

    if (hunk.kind === 'add') {
      const targetPath = resolved || hunk.path
      await fs.writeTextFile(targetPath, hunk.contents)
      recordSummary(summary, seen, 'added', display)
      continue
    }
    ...
    if (hunk.kind === 'delete') {
      ...
      recordSummary(summary, seen, 'deleted', display)
      continue
    }

    const targetPath = resolved || hunk.path
    const applied = await applyUpdateHunk(targetPath, hunk.chunks, {
      readFile: path => fs.readTextFile(path),
    })

    if (hunk.movePath) {
      const moveResolved = resolvePath(hunk.movePath) || hunk.movePath
      await fs.writeTextFile(moveResolved, applied)
      await fs.remove(targetPath)
      recordSummary(summary, seen, 'modified', toDisplayPath(moveResolved))
    }
    else {
      await fs.writeTextFile(targetPath, applied)
      recordSummary(summary, seen, 'modified', display)
    }
  }
